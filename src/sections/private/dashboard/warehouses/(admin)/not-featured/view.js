 ...
 * @returns {JSX.Element}
 */
const WarehouseNotFeaturedView = (props) => {
  ...
  const filteredData = useMemo(
    () =>
      results?.data?.results instanceof Array
        ? results?.data?.results.filter((w) => !w.isFeatured && w?.visible)
        : [],
    [results]
  );
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'xl'}>
      <Stack mb={5} spacing={5}>
        <CustomBreadcrumbs
          heading="Not Featured warehouses"
          links={[
        ...
        <Grid container spacing={2}>
          {renderWarehouses(filteredData, 'No warehouses available')}
        </Grid>
...
WarehouseNotFeaturedView.propTypes = Props;

export default WarehouseNotFeaturedView;
